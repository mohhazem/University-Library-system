...
public class ViewBooks extends javax.swing.JFrame implements Node{
   ...
   backend.LibrarianRole Librarian;
    ...
    public ViewBooks(backend.LibrarianRole Librarian) {
        initComponents();
        String[][] TableData = new String[Librarian.getListOfBooks().length][5];
        for (int i = 0; i < Librarian.getListOfBooks().length; i++) {
            TableData[i] = Librarian.getListOfBooks()[i].lineRepresentation().split(",");
        }
        String columnNames[] = {"Book Id", "Title", "Author Name", "Publisher", "no of copies"};
